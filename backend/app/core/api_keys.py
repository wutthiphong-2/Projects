...
class APIKeyManager:
    ...
    def create_key(
        ...
        try:
            ...
            cursor.execute("""
                ...
                (id, key_hash, name, description, created_by, created_at, 
                 rate_limit_per_minute, rate_limit_per_hour, is_active, permissions)
                VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
            """, (
                key_id, key_hash, name, description, created_by, created_at,
    ...
    def get_key(self, key_id: str) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            cursor.execute("""
                SELECT id, name, description, created_by, created_at, last_used_at,
                       rate_limit_per_minute, rate_limit_per_hour, is_active, permissions
                FROM api_keys
                WHERE id = ?
            """, (key_id,))
